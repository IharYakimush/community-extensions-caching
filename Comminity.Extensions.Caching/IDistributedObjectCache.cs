...
namespace Comminity.Extensions.Caching
{
    ...
    {
        TObject Get<TObject>(string key,Func<byte[], TObject> deserialize = null);
        Task<TObject> GetAsync<TObject>(string key, CancellationToken token = default(CancellationToken), Func<byte[], TObject> deserialize = null);
